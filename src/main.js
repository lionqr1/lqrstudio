...
const PITCH_CONFIG = {
  length: 105, width: 68, goalWidth: 7.32, goalHeight: 2.44, goalDepth: 2,
...
const TEAMS = {
  ...
  awayTeam: {
    name: 'Real Madrid', shortName: 'RMA', primaryColor: '#f1f5f9', secondaryColor: '#d6d6d6', goalkeeperColor: '#00ff00',
    formation: { GK: { x: 0, z: 50 }, LB: { x: 25, z: 35 }, CB1: { x: 8, z: 38 }, CB2: { x: -8, z: 38 }, RB: { x: -25, z: 35 }, CM1: { x: 15, z: 15 }, CM2: { x: 0, z: 20 }, CM3: { x: -15, z: 15 }, LW: { x: 30, z: -10 }, ST: { x: 0, z: -20 }, RW: { x: -30, z: -10 } },
  },
};
...
const ui = {
  ...
  homeName: document.getElementById('home-name'), awayName: document.getElementById('away-name'),
  homeScore: document.getElementById('home-score'), awayScore: document.getElementById('away-score'),
  matchTime: document.getElementById('match-time'), half: document.getElementById('half-indicator'),
  stamina: document.getElementById('stamina-fill'), shot: document.getElementById('shot-fill'),
  indicator: document.getElementById('player-indicator'),
  announcement: document.getElementById('announcement'),
  goalOverlay: document.getElementById('goal-overlay'), goalScorer: document.getElementById('goal-scorer'),
  halftime: document.getElementById('halftime-overlay'), fulltime: document.getElementById('fulltime-overlay'), fulltimeText: document.getElementById('fulltime-text'),
};
...
function announce(message, ms = 1200) {
  ui.announcement.textContent = message;
  ui.announcement.classList.add('visible');
  clearTimeout(announce._timer);
  announce._timer = setTimeout(() => ui.announcement.classList.remove('visible'), ms);
}

function setState(state) {
  gameState = state;
  ui.menu.classList.toggle('visible', state === GAME_STATE.MENU);
  ui.hud.classList.toggle('visible', [GAME_STATE.PLAYING, GAME_STATE.PAUSED, GAME_STATE.GOAL_SCORED].includes(state));
  ui.pause.classList.toggle('visible', state === GAME_STATE.PAUSED);
}

function updateLoadingProgress(percent) { ui.progress.style.width = `${percent}%`; }
...
function createPitchTexture() {
  ...
  cv.width = 2048; cv.height = 1400;
  const ctx = cv.getContext('2d');
  const sx = cv.width / PITCH_CONFIG.length;
  const sz = cv.height / PITCH_CONFIG.width;
  ...
  const paLen = PITCH_CONFIG.penaltyAreaLength * sx;
  const paW = PITCH_CONFIG.penaltyAreaWidth * sz;
  const paTop = (cv.height - paW) / 2;
  ctx.strokeRect(10, paTop, paLen, paW);
  ctx.strokeRect(cv.width - 10 - paLen, paTop, paLen, paW);
...
function createGoal(side) {
  ...
  const top = new THREE.Mesh(new THREE.CylinderGeometry(r, r, w, 12), postMat);
  top.rotation.z = Math.PI / 2; top.position.set(0, h, 0);
  group.add(top);

  const back = new THREE.Mesh(new THREE.PlaneGeometry(w, h), netMat);
  back.position.set(0, h / 2, -d * side);
  group.add(back);

  group.position.z = side * PITCH_CONFIG.halfLength;
...
function createBall() {
  const ball = new THREE.Mesh(
    new THREE.SphereGeometry(GAME_CONFIG.ballRadius, 24, 24),
...
function createSelectionRing() {
  selectionRing = new THREE.Mesh(
    new THREE.RingGeometry(0.43, 0.55, 32),
    new THREE.MeshBasicMaterial({ color: 0xfff176, transparent: true, opacity: 0.95, side: THREE.DoubleSide })
  );
...
function createGeneratedPlayer(team, position, positionName, isGoalkeeper = false) {
  ...
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.24, 0.2, 0.72, 8), jersey);
  body.position.y = 0.9; body.castShadow = true;
  const short = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.18, 0.3, 8), shorts);
  short.position.y = 0.53; short.castShadow = true;
  ...
  group.userData = {
    ...
    isControlled: false,
    stamina: 100,
    speed: isGoalkeeper ? 7 : PLAYER_CONFIG.runSpeed,
...
function createPlayers() {
  ...
  for (const [name, pos] of Object.entries(TEAMS.homeTeam.formation)) {
    homePlayers.push(createGeneratedPlayer(TEAMS.homeTeam, pos, name, name === 'GK'));
  }
...
function switchToPlayer(player) {
  ...
  if (player) {
    ...
    ui.indicator.textContent = `Controlled: ${player.userData.positionName}`;
  }
}
...
function switchPlayer() {
  ...
  announce(`Switched: ${candidates[0].userData.positionName}`, 750);
}
...
function updatePlayerMovement(dt) {
  ...
  if (keys.ShiftLeft || keys.ShiftRight) {
    ...
    controlledPlayer.userData.stamina = Math.min(100, controlledPlayer.userData.stamina + 14 * dt);
  }
  ...
  ui.stamina.style.width = `${controlledPlayer.userData.stamina}%`;
}

function executePass(passer, targetPosition, lift = 1) {
  passer.userData.hasBall = false;
  const dir = targetPosition.clone().sub(gameBall.position);
  ...
  gameBall.userData.velocity.copy(
    dir.normalize().multiplyScalar(Math.min(PLAYER_CONFIG.kickPower * 0.7, dist * 1.4))
  );
...
function pass() {
  ...
  homePlayers.filter((p) => p !== passer && !p.userData.isGoalkeeper).forEach((mate) => {
    ...
    const d = to.length();
    const score = facing.dot(to.normalize()) * 100 - d;
    if (d < 40 && score > bestScore) {
      best = mate;
      bestScore = score;
    }
  });
...
function throughBall() {
  if (!playerHasBall()) return;
  const target = homePlayers
    .filter((p) => ['ST', 'LW', 'RW'].includes(p.userData.positionName))
...
function shoot(power = PLAYER_CONFIG.kickPower, shooter = controlledPlayer) {
  ...
  const target = new THREE.Vector3(
    (Math.random() - 0.5) * PITCH_CONFIG.goalWidth * 0.85,
    Math.random() * PITCH_CONFIG.goalHeight * 0.7 + 0.5,
    goalZ
  );

  const dir = target.sub(gameBall.position).normalize();
  dir.x += (Math.random() - 0.5) * (1 - PLAYER_CONFIG.shotAccuracy) * 0.6;
  gameBall.userData.velocity.copy(dir.normalize().multiplyScalar(power));
  gameBall.userData.velocity.y += clamp(power * 0.16, 2.2, 6.2);
}
...
function tackle() {
  ...
  if (controlledPlayer.position.distanceTo(holder.position) < 2) {
    if (Math.random() < 0.6) {
      ...
      gameBall.userData.lastTouchedBy = controlledPlayer;
      announce('Tackle won!', 800);
    } else {
      holder.userData.hasBall = false;
      gameBall.userData.velocity.set((Math.random() - 0.5) * 10, 2, (Math.random() - 0.5) * 10);
      announce('Loose ball!', 600);
    }
  }
}
...
function checkBallControl() {
  ...
  let nearest;
  let best = PLAYER_CONFIG.controlRadius;
...
function updateOutfieldAI(player, dt, userTeam) {
  ...
  if (player.userData.hasBall && !userTeam) {
    const distanceToGoal = Math.abs(PITCH_CONFIG.halfLength - Math.abs(player.position.z));
    if (distanceToGoal < 24 && Math.random() < 0.025) {
      shoot(PLAYER_CONFIG.kickPower * 0.9, player);
      return;
    }
    if (Math.random() < 0.02) {
      const mate = awayPlayers
        .filter((p) => p !== player && !p.userData.isGoalkeeper)
  ...
  if (!teamHasBall) {
    target.add(gameBall.position.clone().sub(player.position).multiplyScalar(0.25));
  } else {
    target.add(new THREE.Vector3(0, 0, userTeam ? -4 : 4));
  }
...
function updateAI(dt) {
  ...
  homePlayers.forEach((p) => {
    if (p !== controlledPlayer) {
      p.userData.isGoalkeeper ? updateGoalkeeperAI(p, dt) : updateOutfieldAI(p, dt, true);
    }
  });
}
...
function checkGoal() {
  ...
  if (!inX || !inY) return;
  if (gameBall.position.z < -PITCH_CONFIG.halfLength) goalScored('away');
  else if (gameBall.position.z > PITCH_CONFIG.halfLength) goalScored('home');
}
...
function updateBallPhysics(dt) {
  ...
  gameBall.rotation.x += vel.z * dt * 3;
  gameBall.rotation.z -= vel.x * dt * 3;

  const halfW = PITCH_CONFIG.halfWidth;
  if (Math.abs(gameBall.position.x) > halfW && Math.abs(gameBall.position.z) < PITCH_CONFIG.halfLength) {
    gameBall.position.x = clamp(gameBall.position.x, -halfW, halfW);
    vel.x *= -0.7;
  }

  if (Math.abs(gameBall.position.z) > PITCH_CONFIG.halfLength + 3) {
...
function updateMatchTime(dt) {
  ...
  if (matchTime >= GAME_CONFIG.matchDuration) {
    ...
    ui.fulltimeText.textContent = msg;
    ui.fulltime.classList.add('visible');
  }

  const t = Math.floor(matchTime);
  ui.matchTime.textContent = `${String(Math.floor(t / 60)).padStart(2, '0')}:${String(t % 60).padStart(2, '0')}`;
...
function updateRadar() {
  ...
  const sx = radarCanvas.width / PITCH_CONFIG.length;
  const sz = radarCanvas.height / PITCH_CONFIG.width;

  ctx.fillStyle = '#1a4d1a';
  ctx.fillRect(0, 0, radarCanvas.width, radarCanvas.height);
  ctx.strokeStyle = 'rgba(255,255,255,.35)';
  ctx.strokeRect(5, 5, 190, 120);
  ...
  homePlayers.forEach((p) => {
    const rp = toRadar(p.position);
    ctx.beginPath();
    ctx.arc(rp.x, rp.y, p.userData.isControlled ? 4 : 3, 0, Math.PI * 2);
    ctx.fill();
  });
  ...
  ctx.fillStyle = '#fff';
  const bp = toRadar(gameBall.position);
  ctx.beginPath();
...
function updateCamera() {
  ...
  const lookAheadZ = clamp(ballVel.z * 0.3, -8, 8);
  const lookAheadX = clamp(ballVel.x * 0.4, -7, 7);
  const idealTarget = new THREE.Vector3(gameBall.position.x + lookAheadX, 0.7, gameBall.position.z + lookAheadZ);
...
function updateSelectionRing() {
  ...
  selectionRing.position.set(controlledPlayer.position.x, 0.03, controlledPlayer.position.z);
}

function releaseChargedShot() {
  if (!isChargingShot || !playerHasBall()) {
  ...
  const ratio = clamp(shotCharge / GAME_CONFIG.maxShotCharge, 0, 1);
  const power = GAME_CONFIG.minShotPower + (GAME_CONFIG.maxShotPower - GAME_CONFIG.minShotPower) * ratio;
  shoot(power);
  announce(ratio > 0.85 ? 'Power Shot!' : 'Shot!', 700);
  isChargingShot = false;
  shotCharge = 0;
  ui.shot.style.width = '0%';
}
...
function setupUI() {
  ...
  document.getElementById('exhibition-btn').onclick = () => {
    setState(GAME_STATE.PLAYING);
    resetForKickoff('home');
  };
  document.getElementById('settings-btn').onclick = () => {
    alert('Run with: npm install && npm run dev, then open http://localhost:3000');
  };
  ...
  document.getElementById('play-again-btn').onclick = () => { restartMatch(); setState(GAME_STATE.PLAYING); };
}
...
function animate() {
  ...
  if (gameState === GAME_STATE.PLAYING) {
    if (isChargingShot) {
      shotCharge = clamp(shotCharge + dt, 0, GAME_CONFIG.maxShotCharge);
...
async function initGame() {
  ...
  setTimeout(() => {
    ...
    announce('Press Quick Match to start', 1300);
  }, 350);
